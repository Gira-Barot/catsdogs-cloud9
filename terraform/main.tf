...
resource "aws_instance" "linux_machine" {
  ...
  key_name               = aws_key_pair.giraassignment.key_name
  instance_type          = "t2.small"
  vpc_security_group_ids = [aws_security_group.sg-835.id]
  user_data              = file("dockerscript.sh")
  tags = {
    Name = "LinuxServer-machine"
  }
}

resource "aws_security_group" "sg-835" {
...
resource "aws_key_pair" "giraassignment" {
  key_name   = "giraassignment"
